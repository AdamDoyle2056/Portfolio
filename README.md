# Adam Doyle — Game Developer & Creative Software Engineer  

[![GitHub followers](https://img.shields.io/github/followers/adam-doyle?label=Followers&style=social)](https://github.com/adam-doyle)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Profile-blue?logo=linkedin)](https://linkedin.com/in/your-link)
[![itch.io](https://img.shields.io/badge/itch.io-Games-red?logo=itch.io)](https://your-itch-page.itch.io)

---

## About Me  

I’m a final-year Computer Science student passionate about game development and creative software engineering.  
My focus is on building immersive gameplay systems, efficient code architectures, and clean, well-documented projects.

I’m currently:  
- Completing my final-year CS coursework  
- Developing a capstone Unity game (Weeks 5–10 of my summer roadmap)  
- Preparing a portfolio to showcase technical and creative projects  

I’m open to opportunities in game development, tools engineering, or creative software roles where I can blend art, code, and systems thinking.

---

## Projects  

| Project | Description | Tech Stack | Link |
|----------|--------------|-------------|------|
| Capstone Game (In Progress) | A Unity-based roguelike with modular systems, procedural challenges, and tight gameplay loops. | Unity • C# • Git • ScriptableObjects | [Repo](https://github.com/adam-doyle/capstone-game) |
| Procedural Terrain Tool | Generates stylized terrain using Perlin noise and texture mapping. | Unity • C# • Editor Tools | [Repo](https://github.com/adam-doyle/terrain-tool) |
| Inventory & Item System | Modular Unity package for drag/drop, stacking, and item data using ScriptableObjects. | Unity • C# • UI Toolkit | [Repo](https://github.com/adam-doyle/inventory-system) |
| Utility Scripts Collection | A set of reusable Unity utilities and performance helpers. | Unity • C# | [Repo](https://github.com/adam-doyle/unity-utils) |

---

## Skills & Tools  

**Game Development:** Unity, C#, Cinemachine, Shader Graph, Editor Tools  
**Software Engineering:** Git, GitHub Projects, .NET, Python (automation/scripts)  
**Design & Architecture:** Game architecture, UI/UX feedback systems, performance optimization  
**Workflows:** Agile development, Markdown documentation, version control best practices  

---

## Portfolio Showcase  

*(Replace these with screenshots or GIFs of your projects. GitHub supports embedded images.)*  

| Project Preview | |
|:----------------:|:----------------:|
| ![Capstone](images/capstone.gif) | ![Terrain Tool](images/terrain_tool.gif) |

---

## Contact  

- LinkedIn: [linkedin.com/in/your-link](https://linkedin.com/in/your-link)  
- Resume: [your-website.com/resume.pdf](https://your-website.com/resume.pdf)  
- Email: youremail@example.com  
- itch.io: [your-itch-page.itch.io](https://your-itch-page.itch.io)

---

## Currently Working On  

- Finalizing the Unity capstone (Weeks 5–10 of my roadmap)  
- Enhancing my GitHub portfolio and technical documentation  
- Learning advanced C# design patterns for scalable Unity systems  

---

> "Great games are built on clear systems and creative curiosity."

---

